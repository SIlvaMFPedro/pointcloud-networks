...
def get_transform_K(inputs, is_training, bn_decay=None, k=3):
    """
        Get Transform K Network
        :param inputs: BxNx1xK gray image
        :param is_training: training flag
        :param bn_decay: decay flag
        :param K: k parameter
        :return: transformation matrix of size KxK
    """
    batch_size = inputs.get_shape()[0].value
    num_point = inputs.get_shape()[1].value
    net = tf_util.conv2d(inputs, 256, [1, 1], padding='VALID', stride=[1, 1], bn=True, is_training=is_training,
...
def get_transform(point_cloud, is_training, bn_decay=None, k = 3):
    """
        Get Transform Network
        :param point_cloud: BxNx3 gray image
        :param is_training: training flag
        :param bn_decay: decary flag
        :param k: k parameter
...
def get_model(point_cloud, input_label, is_training, cat_num, part_num, batch_size,
    ...
    out1 = tf_util.conv2d(input_image, 64, [1, K], padding='VALID', stride=[1, 1], bn=True, is_training=is_training,
                          scope='conv1', bn_decay=bn_decay)
    out2 = tf_util.conv2d(out1, 128, [1, 1], padding='VALID', stride=[1, 1], bn=True, is_training=is_training,
                          scope='conv2', bn_decay=bn_decay)
    out3 = tf_util.conv2d(out2, 128, [1, 1], padding='VALID', stride=[1, 1], bn=True, is_training=is_training,
    ...
    out4 = tf_util.conv2d(net_transformed, 512, [1, 1], padding='VALID', stride=[1, 1], bn=True, is_training=is_training,
                          scope='conv4', bn_decay=bn_decay)
    out5 = tf_util.conv2d(out4, 2048, [1, 1], padding='VALID', stride=[1, 1], bn=True, is_training=is_training,
